...
class Adapter(nn.Module):
    ...
    def forward(self, x):
        h = self.relu(self.fc1(x)) # self.fc1.weight.dtype, h.dtype are float16
        h = self.fc2(h)
        return x + h
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, latent=64, adapter=False):
        ...
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask, latent=latent, adapter=adapter) for _ in range(layers)])
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        return x.type(torch.FloatTensor).to(x.device)
...
class CLIPAdapter(nn.Module):
    ...
    def forward(self, image):
        image_features = self.forward_features(image)

    def adapter_parameters(self):
        return [p for n, p in self.visual.named_parameters() if 'adapter' in n or 'ln_' in n]
