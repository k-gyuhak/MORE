...
class Adapter(nn.Module):
    ...
    def mask(self, t, s):
        # gc1 = self.gate(s * self.ec1[t]).type(torch.HalfTensor).to('cuda')
        # gc2 = self.gate(s * self.ec2[t]).type(torch.HalfTensor).to('cuda')
    ...
    def append_embedddings(self):
        ...
        self.ec1.append(nn.Parameter(torch.randn(1, self.out_dim, dtype=torch.float16, device='cuda')))
        self.ec2.append(nn.Parameter(torch.randn(1, self.in_dim, dtype=torch.float16, device='cuda')))
...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t=None, s=None):
        ...
        return x.type(torch.FloatTensor).to(x.device), list(itertools.chain(*msk))
...
class CLIPAdapter(nn.Module):
    ...
    def forward_features(self, image, t=None, s=None):
        return self.visual(image.type(self.dtype), t, s)
