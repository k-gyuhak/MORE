...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.num_heads = num_heads # THIS IS 12 FOR ALL LAYERS IN vit_base_patch16_224
        head_dim = dim // num_heads
        self.scale = head_dim ** -0.5 # WHAT IS THIS? 
...
class MyVisionTransformer(nn.Module):
    ...
    def adapter_parameters(self):
        if self.freeze_head:
            return [p for n, p in self.named_parameters() if 'adapter' in n or 'norm' in n]
        else:
    ...
    def get_params(self) -> torch.Tensor:
        ...
        """
        params = []
        for pp in self.adapter_parameters():
